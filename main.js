...
async function scanDirectory(directoryPath) {
  if (!directoryPath || !(await fileExists(directoryPath))) return { directoryPath, entries:[], missing:true };
  ...
  for (const entry of directoryEntries) {
    ...
    entries.push({
      id:baseName, title:baseName.replace(/[-_]+/g, " "),
      videoPath:pathToFileURL(videoFilePath).href,
      funscriptPath:(await fileExists(funscriptFilePath)) ? pathToFileURL(funscriptFilePath).href : null,
