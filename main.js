...
async function scanVideoMetadata(root, metadataPath, usedIds) {
  ...
  const funscripts = {}; const funscriptDiagnostics = {};
  ...
  return {
    ...
    funscriptDiagnostics, requiresVictory:metadata?.requiresVictory !== false, grantsEncounterReward:metadata?.grantsEncounterReward !== false, canEndRun:metadata?.canEndRun !== false
  };
}
...
async function scanLegacyFlatDirectory(directoryPath) {
  const entries = []; if (!(await fileExists(directoryPath))) return entries;
  for (const entry of await fs.readdir(directoryPath, { withFileTypes:true })) {
    ...
    const videoFilePath = path.join(directoryPath, entry.name); const extension = path.extname(entry.name).toLowerCase(); const baseName = path.basename(entry.name, extension); const funscriptFilePath = path.join(directoryPath, `${baseName}.funscript`); const stat = await fs.stat(videoFilePath); const validationErrors = await validateFunscriptFile(funscriptFilePath); const hasFunscript = await fileExists(funscriptFilePath) && validationErrors.length === 0;
    entries.push({ kind:"video", id:normalizeId(baseName), title:baseName.replace(/[-_]+/g, " "), type:"normal", durationSeconds:0, difficulties:hasFunscript ? ["default"] : [], themes:[], performers:[], enabled:true, weight:1, allowRepeatInSameRun:false, videoPath:pathToFileURL(videoFilePath).href, funscripts:hasFunscript ? { default:pathToFileURL(funscriptFilePath).href } : {}, thumbnailPath:null, available:hasFunscript, errors:hasFunscript ? [] : validationErrors.map((error) => `invalid-funscript:default:${error}`), sizeBytes:stat.size, modifiedAt:stat.mtimeMs, sourceSignature:`${stat.size}:${Math.trunc(stat.mtimeMs)}:${await signature(funscriptFilePath)}` });
  }
  return entries;
}
...
async function importVideo(payload = {}) {
  ...
  if (!id) throw new Error("Impossible de générer un identifiant valide."); if (!(await ensureUniqueId(root, id))) throw new Error(`L'identifiant ${id} existe déjà.`);
  const sourceFunscripts = Array.isArray(payload.funscripts) ? payload.funscripts : []; if (!sourceFunscripts.length) throw new Error("Au moins un funscript est obligatoire.");
  const validated = [];
