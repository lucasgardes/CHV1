...
export class MapView{
 constructor({mapNodeList,goldValue,inventoryValue,getItemById,onNodeSelected}){
  ...
  ensureMapStylesheet();Object.assign(this,{mapNodeList,goldValue,inventoryValue,getItemById,onNodeSelected});this.navigationLocked=false;
 }
 render({gameState,currentNode,accessibleNodes}){
  ...
  const wrapper=document.createElement("div");wrapper.className="map-scroll-area";const graph=document.createElement("div");graph.className="map-graph";graph.style.width=`${MAP_WIDTH}px`;graph.style.height=`${this.getHeight(rows.length)}px`;
 ...
 unlockNavigation(){this.navigationLocked=false;for(const candidate of this.mapNodeList.querySelectorAll(".map-node-button.is-accessible"))candidate.disabled=false;}
 createNode({node,position,currentNode,accessibleNodes,completedNodeIds}){
  ...
  if(accessible)button.addEventListener("click",()=>{if(this.navigationLocked)return;this.navigationLocked=true;for(const candidate of this.mapNodeList.querySelectorAll(".map-node-button.is-accessible"))candidate.disabled=true;try{const result=this.onNodeSelected(node.id);Promise.resolve(result).catch(()=>this.unlockNavigation());}catch(error){this.unlockNavigation();throw error;}window.setTimeout(()=>{if(this.navigationLocked&&this.mapNodeList.contains(button))this.unlockNavigation();},1200);});
  return button;
 }
