...
export class CampfireView {
  ...
  showArchive(archive,{alreadyExplored=false,onContinue=()=>{}}={}){this.mainChoices.hidden=true;this.upgradeList.hidden=false;this.upgradeList.replaceChildren();const card=document.createElement("article");card.className="room-item-card archive-card";const title=document.createElement("h3");title.textContent=archive?.title??"Archive clandestine";const text=document.createElement("p");text.textContent=alreadyExplored?"Tu reconnais cette pièce. Aucun nouveau message n’a été laissé.":archive?.text??"Le message est illisible.";const button=document.createElement("button");button.type="button";button.textContent="Quitter la pièce";button.addEventListener("click",onContinue);card.append(title,text,button);this.upgradeList.append(card);this.message.textContent=alreadyExplored?"Cette archive était déjà connue.":"Nouvelle archive ajoutée aux souvenirs du cobaye 426.";}
}
