...
function normalizeEntry(entry = {}) {
  return {
    ...
    videoPath: entry.videoPath ?? null, funscriptPath: entry.funscriptPath ?? null,
    type: entry.type ?? "normal", difficulty: entry.difficulty ?? "normal",
...
export class LocalMediaLibrary {
  constructor({ recentLimit = 4, random = Math.random } = {}) {
    ...
    this.entries = []; this.recentIds = []; this.directoryPath = null; this.missing = false;
  }
  loadScan(scan = {}) {
  ...
  getCandidates({ type = null, difficulty = null, requireFunscript = true } = {}) {
    return this.entries.filter((entry) => (!type || entry.type === type) && (!difficulty || entry.difficulty === difficulty) && (!requireFunscript || entry.funscriptPath));
  }
  select(options = {}) {
    let candidates = this.getCandidates(options);
    if (!candidates.length && options.difficulty) candidates = this.getCandidates({ ...options, difficulty:null });
    ...
    if (selected) this.recentIds = [selected.id, ...this.recentIds.filter((id) => id !== selected.id)].slice(0, this.recentLimit);
    return selected;
  }
  markUnavailable(entryId) { this.entries = this.entries.filter((entry) => entry.id !== entryId); this.recentIds = this.recentIds.filter((id) => id !== entryId); }
}
