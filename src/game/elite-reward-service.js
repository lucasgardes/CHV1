...
export class EliteRewardService {
  ...
  getRareChance(encounter) {
    ...
    const difficultyBonus=difficulty==="hard"?.12:difficulty==="normal"?.05:0;
    ...
    return Math.min(.75,.05+difficultyBonus+lucky+cursed+eventBonus);
  }
  ...
  createItemReward(ownedItemIds,rareChance){const item=weightedItem(getAvailableItems(ownedItemIds),rareChance,this.random);return item?{type:"item",itemId:item.id,item}:null;}
  createBundle(encounter={}) {
    const firstElite=(this.gameState.elitesDefeated??0)===0;
    const rareChance=this.getRareChance(encounter);
    const rewards=[]; const reservedIds=[...this.gameState.inventory];
    const addItem=()=>{const reward=this.createItemReward(reservedIds,rareChance);if(!reward)return false;rewards.push(reward);reservedIds.push(reward.itemId);return true;};
    if(firstElite)addItem();
    while(rewards.length<2){const itemChance=firstElite?.35:.45;if(this.random()<itemChance&&addItem())continue;rewards.push(this.createGoldReward(encounter));}
    return{rewards,firstElite,rareChance,encounterDifficulty:inferDifficulty(encounter)};
  }
  applyBundle(bundle,{applyGold=true}={}) {
    const applied=[];
    for(const reward of bundle?.rewards??[]){
      if(reward.type==="gold"){if(applyGold)this.gameState.addGold(reward.amount);applied.push(reward);continue;}
      if(reward.type==="item"&&this.gameState.addItem(reward.itemId)){this.itemController?.ensureRuntimeState(reward.itemId);applied.push(reward);}
    }
