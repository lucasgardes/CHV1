...
export class PhaseOneController{
 ...
 completeBoss(){const hasEscapeKey=this.gameState.hasItem("last-act-key")||this.gameState.hasItem("escape-prototype");const ending=hasEscapeKey?ENDINGS.GOOD:ENDINGS.BAD;let meta=this.saveService.update((current)=>({...current,victories:current.victories+1,lastEnding:ending,endingsSeen:[...new Set([...current.endingsSeen,ending])],unlockedContent:ending===ENDINGS.GOOD?[...new Set([...current.unlockedContent,"true-ending","archives-menu"])]:current.unlockedContent}));const archiveReward=this.archiveService.unlockSecretAfterBoss();if(archiveReward.unlocked)meta=archiveReward.meta;this.gameState.completeCurrentNode();this.gameState.setCurrentEncounter(null);this.gameState.setStatus(GAME_STATUS.VICTORY);this.onTransition({type:"ending",ending,meta,archiveReward});return{ending,meta,archiveReward};}
