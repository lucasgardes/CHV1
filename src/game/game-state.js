...
function createItemRunState(){return{metronomeStreak:0,rescueUsedInEncounter:false,doubleCommandUsed:false,doubleCommandPendingItemId:null,fastPathRows:[],recyclerReady:true,directorChoice:null,utilitySlotsUsed:[],encounterHistory:[],playedMediaIds:[]};}
export class GameState{
 ...
 registerCampfireVisit(nodeId){if(!nodeId||this.campfireNodeIdsSeen.includes(nodeId))return false;this.campfireNodeIdsSeen.push(nodeId);this.campfiresEncountered+=1;return true;}
 ...
 queueDeferredEncounterReward(reward={}){const normalized={id:String(reward.id||`deferred-${Date.now()}-${this.deferredEncounterRewards.length}`),source:reward.source??"event",encountersRemaining:Math.max(1,Number(reward.encounters)||1),gold:Math.max(0,Number(reward.gold)||0)};this.deferredEncounterRewards.push(normalized);return{...normalized};}
 advanceDeferredEncounterRewards(){const granted=[];const pending=[];for(const reward of this.deferredEncounterRewards){const next={...reward,encountersRemaining:reward.encountersRemaining-1};if(next.encountersRemaining<=0){const gold=this.addGold(next.gold);granted.push({...next,gold});}else pending.push(next);}this.deferredEncounterRewards=pending;return granted;}
 getDeferredEncounterRewards(){return this.deferredEncounterRewards.map((reward)=>({...reward}));}
 revealMapEncounters(encounters=[]){const revealed=[];for(const encounter of encounters){if(!encounter?.id)continue;const snapshot={id:encounter.id,title:encounter.title??"Rencontre",type:encounter.type??"normal",themes:Array.isArray(encounter.themes)?[...encounter.themes]:[],difficulty:encounter.difficulty??null,durationSeconds:Number(encounter.durationSeconds)||0,showDetails:encounter.showDetails===true,showTheme:encounter.showTheme!==false,revealedAtNodeId:this.currentNodeId};this.revealedMapEncounters[encounter.id]=snapshot;revealed.push({...snapshot,themes:[...snapshot.themes]});}return revealed;}
 getMapEncounterReveal(nodeId){const reveal=this.revealedMapEncounters[nodeId];return reveal?{...reveal,themes:[...(reveal.themes??[])]}:null;}
 isMapEncounterRevealed(nodeId){return Boolean(this.revealedMapEncounters[nodeId]);}
 ...
 multiplyNextShopPrices(multiplier){this.nextShopPriceMultiplier=Math.max(.1,this.nextShopPriceMultiplier*multiplier);}
 ...
 addGold(amount,{ignoreBlessing=false}={}){if(!Number.isFinite(amount)||amount<0)throw new Error("La quantité d’or ajoutée est invalide.");const multiplier=!ignoreBlessing&&this.activeBlessingId==="revenant-fortune"?1.15:1;const applied=Math.round(amount*multiplier);this.gold+=applied;this.totalGoldEarned+=applied;return applied;}
