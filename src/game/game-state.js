...
export class GameState{
 ...
 revealMapEncounters(encounters=[]){const revealed=[];for(const encounter of encounters){if(!encounter?.id)continue;const snapshot={id:encounter.id,title:encounter.title??"Rencontre",type:encounter.type??"normal",difficulty:encounter.difficulty??null,durationSeconds:Number(encounter.durationSeconds)||0,revealedAtNodeId:this.currentNodeId};this.revealedMapEncounters[encounter.id]=snapshot;revealed.push({...snapshot});}return revealed;}
 getMapEncounterReveal(nodeId){const reveal=this.revealedMapEncounters[nodeId];return reveal?{...reveal}:null;}
