...
export class MapController {
  ...
  getNodeById(nodeId) { return this.nodesById.get(nodeId) ?? null; }
  getCurrentNode() { return this.getNodeById(this.gameState.currentNodeId); }
  getAccessibleNodes() {
    const currentNode = this.getCurrentNode();
    const accessibleNodes = currentNode === null ? [] : currentNode.nextNodeIds.map((nodeId) => this.getNodeById(nodeId)).filter((node) => node !== null && node.hidden !== true);
    Object.defineProperties(accessibleNodes, {
      mapRows: { value: this.getRows(), enumerable: false }, mapNodes: { value: this.getNodes(), enumerable: false },
      mapSeed: { value: this.map.seed, enumerable: false }, validation: { value: this.map.validation, enumerable: false }
    });
    return accessibleNodes;
  }
  isNodeAccessible(nodeId) { return this.getAccessibleNodes().some((node) => node.id === nodeId); }
  moveToNode(nodeId) {
    const targetNode = this.getNodeById(nodeId);
    if (targetNode === null || !this.isNodeAccessible(nodeId)) throw new Error(`La case ${nodeId} n’est pas accessible.`);
    const currentNode = this.getCurrentNode();
    if (currentNode !== null && currentNode.type === "start") this.gameState.completeCurrentNode();
